...
- **A** および `-` 長押しで SCROLL。**`/`** 長押しで ARROW（暫定）
...
左手 Cmd はプレーン。右最内は Cmd tap=かな。右は内側から Cmd / Enter / Ctrl / BSPC。ARROW の本配置は未定（暫定で `/` 長押し）。Tab は combo `A+S`。

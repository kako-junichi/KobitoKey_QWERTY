# KobitoKey_QWERTY

KobitoKey のカスタムキーマップ（kako-junichi 用）。
ZMK ベース。操作モデルは [zmk-config-roBa](https://github.com/kako-junichi/zmk-config-roBa) に合わせ、両手ボールと A 長押しスクロールは残している。

凡例: `↦` = 長押しでレイヤー切替。

## レイヤー一覧


| Layer | 名前     | 進入方法                                    |
| ----- | ------ | --------------------------------------- |
| 0     | MAC    | デフォルト                                   |
| 1     | WIN    | BT レイヤーで切替                              |
| 2     | NUM    | 左親指 SPACE 長押し                           |
| 3     | ARROW  | 予備（矢印は NUM の HJKL）                        |
| 4     | MOUSE  | どちらかのボールを動かすと自動進入（5秒滞留、roba と同じ）        |
| 5     | SCROLL | A もしくは `-` 長押し                          |
| 6     | BT     | 左親指 Ctrl + Shift コンボ                    |
| 7     | SYM    | 右親指 ENTER 長押し（内側から2番目）            |
| 8     | FUNC   | SPACE + ENTER 同時押し（NUM+SYM の tri-layer） |
| 9     | AERO   | 右親指 BSPC 長押し（内側から4番目）             |


## デフォルトレイヤー（Mac）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌─────┬─────┬─────┬─────┬─────┐
│  Q  │  W  │  E  │  R  │  T  │  │  Y  │  U  │  I  │  O  │  P  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│A/SCR│  S  │  D  │  F  │  G  │  │  H  │  J  │  K  │  L  │-/SCR│
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  Z  │  X  │  C  │  V  │  B  │  │  N  │  M  │  ,  │  .  │  /  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│LALT │LCTRL│LSFT │ SPC │ CMD │  │ CMD │ ENT │CTRL │BSPC │ ss  │
│     │     │     │/NUM │     │  │/かな│/SYM │     │/AERO│     │
└─────┴─────┴─────┴─────┴─────┘  └─────┴─────┴─────┴─────┴─────┘
```

- **A** および `-` 長押しで SCROLL。矢印は SPACE 長押し（NUM）の右手 HJKL
- `;` `'` は内列が無いので SYM へ。`=` は combo `C+V`、`"` は combo `L+-`
- IME は右最内 Cmd の tap=かな。英数は combo D+F（J+K でもかな）。左手 Cmd はプレーン



### 親指キー


| 位置  | キー        | 動作                                               |
| --- | --------- | ------------------------------------------------ |
| 30  | LALT      | Alt（一番外側。Raycast の Option+Space 用）               |
| 31  | LCTRL     | Ctrl（外側寄り。Shift と両方使うので端には置かない）             |
| 32  | LSFT      | Shift（内側。Space の左）                                  |
| 33  | SPACE/NUM | tap=SPACE / hold=NUM（内側）                         |
| 34  | LCMD      | Cmd（左手のみ。プレーン）                                  |
| 35  | RCMD/かな   | tap=かな / hold=Cmd（右・最内）                          |
| 36  | ENTER/SYM | tap=Enter / hold=SYM                                   |
| 37  | LCTRL     | Ctrl（右手）                                            |
| 38  | BSPC/AERO | tap=BackSpace / hold=AERO                              |
| 39  | ss_ht     | tap=Cmd+Shift+4 / hold=Cmd+Shift+5（外側）           |


左手 Cmd はプレーン。右最内は Cmd tap=かな。右は内側から Cmd / Enter / Ctrl / BSPC。矢印は NUM の HJKL。Tab は combo `A+S`。

## NUMレイヤー（SPACE 長押し）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌─────┬─────┬─────┬─────┬─────┐
│  -  │  1  │  2  │  3  │  +  │  │Ctl+1│Ctl+2│Ctl+←│Ctl+↑│Ctl+→│
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  /  │  4  │  5  │  6  │  0  │  │  ←  │  ↓  │  ↑  │  →  │  -  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│*(⇧) │  7  │  8  │  9  │  .  │  │  -  │  -  │  -  │  -  │  -  │
└─────┴─────┴─────┴─────┴─────┘  └─────┴─────┴─────┴─────┴─────┘
```

- 左手 = 電卓配列（キーパッド数字）
- 右手上段 = Spaces（Ctrl+1 / Ctrl+2 / 左デスクトップ / Mission Control / 右デスクトップ）
- 右手ホーム HJKL = 矢印（vim と同じ）
- Ctrl+3〜5 は上段を矢印ナビに譲ったので無し
- 記号は SYM（ENTER 長押し）へ分離



## ARROWレイヤー（予備。常用は NUM の HJKL）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌─────┬─────┬─────┬─────┬─────┐
│ ESC │前ﾀﾌﾞ│  ↑  │次ﾀﾌﾞ│  -  │  │  -  │  -  │  -  │  -  │  -  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│Home │  ←  │  ↓  │  →  │ End │  │  ←  │  ↓  │  ↑  │  →  │  -  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│ ⇧   │  -  │  -  │  -  │  -  │  │  -  │  -  │  -  │  -  │  -  │
└─────┴─────┴─────┴─────┴─────┘  └─────┴─────┴─────┴─────┴─────┘
```

- 左手 `E/S/D/F` = 逆T字の矢印、右手 `H/J/K/L` = vim 風
- 前タブ = Ctrl+Shift+Tab、次タブ = Ctrl+Tab
- 常用の矢印は NUM（SPACE 長押し）の HJKL。このレイヤー自体は今は使わない




## SYMレイヤー（ENTER 長押し）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌─────┬─────┬─────┬─────┬─────┐
│  !  │  @  │  #  │  $  │  %  │  │  ^  │  &  │  *  │  (  │  )  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  `  │  ~  │  _  │  =  │  +  │  │  -  │  [  │  ]  │  {  │  }  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  ;  │  '  │  <  │  >  │  \  │  │  |  │  -  │  -  │  -  │  -  │
└─────┴─────┴─────┴─────┴─────┘  └─────┴─────┴─────┴─────┴─────┘
```

- 上段は US 数字段の Shift と同じ並び
- `;` `'` は roba 内列の代替



## FUNCレイヤー（SPACE + ENTER 同時押し）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌─────┬─────┬─────┬─────┬─────┐
│輝度-│輝度+│  -  │  -  │  -  │  │ F1  │ F2  │ F3  │ F4  │ F5  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│ミュート│音量-│音量+│  -  │  -  │  │ F6  │ F7  │ F8  │ F9  │ F10 │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  -  │  -  │  -  │  -  │  -  │  │  -  │  -  │  -  │ F11 │ F12 │
└─────┴─────┴─────┴─────┴─────┘  └─────┴─────┴─────┴─────┴─────┘
```



## AEROレイヤー（BSPC 長押し / AeroSpace）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌─────┬─────┬─────┬─────┬─────┐
│ WS1 │ WS2 │ WS3 │ WS4 │ WS5 │  │直前 │全画面│ﾈｲﾃｨﾌﾞ│ﾘｻｲｽﾞ│分割 │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│移動1│移動2│移動3│移動4│移動5│  │  ←  │  ↓  │  ↑  │  →  │  -  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  -  │  -  │  -  │  -  │  -  │  │  ←  │  ↓  │  ↑  │  →  │ﾚｲｱｳﾄ│
└─────┴─────┴─────┴─────┴─────┘  └─────┴─────┴─────┴─────┴─────┘
```


| 位置           | 動作                                  |
| ------------ | ----------------------------------- |
| 左上段 Q〜T      | ワークスペース 1〜5（Alt+1〜5）                |
| 左ホーム A〜G     | ウィンドウを WS へ移動（Alt+Shift+1〜5）        |
| 右ホーム H/J/K/L | フォーカス移動（Alt+H/J/K/L）                |
| 右下段 N/M/,/.  | ウィンドウ移動（Alt+Shift+H/J/K/L、Shift 不要） |
| Y            | 直前 WS（Alt+Tab）                      |
| U            | フルスクリーン（Alt+F）                      |
| I            | macOS native fullscreen（Alt+M）      |
| O            | リサイズモード（Alt+R）                      |
| P            | 分割向き切替（Alt+,）                       |
| /            | tiles ⇔ accordion（Alt+/）            |


AeroSpace 設定本体は `~/.config/aerospace/aerospace.toml`。このレイヤーは Alt+◯◯ を送るだけなので toml 側の変更は不要。

## BTレイヤー（左親指 Ctrl + Shift）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌──────┬──────┬─────┬──────┬──────────┐
│  -  │  -  │  -  │  -  │  -  │  │bt_mac│bt_win│ BT2 │ BT3  │   BT4    │
├─────┼─────┼─────┼─────┼─────┤  ├──────┼──────┼─────┼──────┼──────────┤
│  -  │  -  │  -  │  -  │  -  │  │  -   │  -   │  -  │  -   │    -     │
├─────┼─────┼─────┼─────┼─────┤  ├──────┼──────┼─────┼──────┼──────────┤
│  -  │  -  │  -  │  -  │  -  │  │  -   │  -   │  -  │  -   │  BT_CLR  │
└─────┴─────┴─────┴─────┴─────┘  └──────┴──────┴─────┴──────┴──────────┘
```

- 右親指 SS 位置 = BT_CLR_ALL
- `bt_mac` = BT0 + Mac レイヤー固定、`bt_win` = BT1 + Win レイヤー固定



## MOUSEレイヤー（ボール動作で自動進入、5秒滞留）

```
┌─────┬─────┬─────┬─────┬─────┐  ┌─────┬─────┬─────┬─────┬─────┐
│  -  │  -  │  -  │  -  │  -  │  │  -  │  -  │  -  │  -  │次画面│
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  -  │ MB1 │ MB3 │ MB2 │  -  │  │  -  │ MB1 │ MB3 │ MB2 │  -  │
├─────┼─────┼─────┼─────┼─────┤  ├─────┼─────┼─────┼─────┼─────┤
│  -  │  -  │  -  │  -  │  -  │  │戻る │  -  │  -  │進む │  -  │
└─────┴─────┴─────┴─────┴─────┘  └─────┴─────┴─────┴─────┴─────┘
```

- 左手 S/D/F = MB1/MB3/MB2、右手 J/K/L = MB1/MB3/MB2（両手クリックは KobitoKey 側の利点）
- N = Cmd+[（戻る）/ `.` = Cmd+]（進む）/ `P` = 次ディスプレイ（Ctrl+Alt+N）
- `-` は MOUSE 中も透過のまま。ボール滞留中に `-` を長押ししても SCROLL に入れる
- クリック・戻る/進む・次ディスプレイは滞留時間延長から除外



## SCROLLレイヤー（A or `-` 長押し）

- 全キー透過
- **どちらのボールもスクロール挙動になる**
- A 離す / `-` 離す → 通常モードへ復帰
- スクロール量は両ボールとも CPI 400 相当（roba の `SCROLL_TICK=32` 基準）。左 200 CPI は 1/16、右 600 CPI は 1/48。ポインタ CPI は変えない



## コンボ一覧


| キー           | 動作             | 用途                     |
| ------------ | -------------- | ---------------------- |
| Q + W        | ESC            | キャンセル/Vim              |
| A + S        | TAB            | タブキー                   |
| D + F        | 英数（LANG2）      | IME 英数                 |
| J + K        | かな（LANG1）      | IME かな                 |
| L + `-`      | `"`            | ダブルクォート                |
| C + V        | `=`            | イコール                   |
| U + I        | Ctrl+Shift+Tab | 前のタブ                   |
| I + O        | Ctrl+Tab       | 次のタブ                   |
| M + `,`      | Cmd+[          | ブラウザ戻る                 |
| `,` + `.`    | Cmd+]          | ブラウザ進む                 |
| Ctrl + Shift | BT レイヤー        | Bluetooth / Mac・Win 切替 |


タイムアウト: 50ms

## ホストツール側ショートカット



### Raycast（推奨）

- `Option+Space` で Raycast 起動（左親指 LALT + SPACE）
- "Switch Windows" コマンド → 検索付きウィンドウ切替



### AeroSpace

キー本体のバインドは従来どおり Alt+◯◯。KobitoKey からは **BSPC 長押し（AERO）** で Option を意識せず叩ける。
